...
5. **Pointers to `agent_notes/` removed.** Sentences that pointed to `agent_notes/*.md` for full
   ...
   `agent_notes/fathooks.md`, which `repo_README.md` points to. If the repository is published
...
7. **Placeholders.** `[NAMES]` and `[REPO URL]` in `manuscript.md` (provenance block); `[NAME]`,
   `[REPO URL]`, `[EMAIL]` in `cover_email.txt`; `[NAMES]` in `repo_README.md`. The PDF must be
